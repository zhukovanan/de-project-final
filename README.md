...
- создаем витрины в `STAGING`- и `DWH`-слоях
    - STAGING:
       - stg_transactions - данные по транзакциям за предыдущий день
       - stg_currencies - данные по курсу валют за предыдуший день
    - DWH:
       - dwh_global_metrics - итоговая витрина по данным, которая отражает результативность каждого дня, аналогично наполняется инкрементально за предыдуший день
- забираем файлы из репозитория в S3
- прогружаем скаченные файлы в соответствующие таблицы слоя `STAGING` инкрементально
- запускаем расчет показателей для витрины dwh_global_metrics и добавляем расчетные метрики за предыдущий день
